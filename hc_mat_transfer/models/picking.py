...
class ImportPicking(models.TransientModel):
    ...
    location_id = fields.Many2one(
        'stock.location', "Source Location Zone", required=True, compute='_default_location')
    location_dest_id = fields.Many2one(
        'stock.location', "Destination Location Zone", required=True, compute='_default_dest_location'
    )
    transfer_template = fields.Binary('Template', compute="_get_transfer_template")

    def _default_location(self):
        for rec in self:
            rec.location_id = self.picking_type_id.default_location_src_id.id

    def _default_dest_location(self):
        for rec in self:
            rec.location_dest_id = self.picking_type_id.default_location_dest_id.id
    ...
    def make_picking_line(self, values, pick_id):
        ...
        print("values.get('default_code')", values.get('default_code'))
    ...
    def import_picking(self):
        ...
        if not self.file:
            ...
            list_col = ['INTERNAL REFERENCE', 'LOT NUMBER', 'EXPIRY DATE', 'QUANTITY']
            for row_no in range(sheet.nrows):
                ...
                if row_no <= 0:
                    ...
                    if line[2] != '':
                        ...
                        expiry_date_string = xlrd.xldate.xldate_as_tuple(expiry_date_float, datemode=0)
                    else:
